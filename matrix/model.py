...
class GraphReasoningModel(nn.Module):
    ...
    def forward(self, heads, questions, answers=None, entity_range=None, origin_question=None):
        ...
        for t in range(self.num_steps):
            ...
            q_dist = q_dist / (torch.sum(q_dist, dim=1, keepdim=True) + 1e-6)
        ...
        # index = hop_prob.max(1)[1].squeeze()
        # hop_onehot = torch.zeros_like(hop_attn).to(device)
        # for i in range(bsz):
        #     hop_onehot[i, index[i], 0] = 1
