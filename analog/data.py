...
def collate(batch):
    ...
    topic_entity, question, answer, triples, entity_range, origin_question = batch
    ...
    return topic_entity, question, answer, triples, entity_range, origin_question
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        topic_entity, question, answer, triples, entity_range, origin_question = self.questions[index]
...
class DataLoader(torch.utils.data.DataLoader):
    def __init__(self, fn, bert_name, ent2id, rel2id, batch_size, id2name, add_rev=False, training=False):
        ...
        for line in open(fn):
            ...
            origin_question = instance['question'].strip()
            question = self.tokenizer(instance['question'].strip(), max_length=300, padding='max_length', return_tensors="pt")
            ...
            is_bad = False  # 子图应当包含头尾实体，也就是说关于子图提取是用的别人提取好的子图
            ...
            data.append([head, question, ans, triples, entity_range, origin_question])   # 相比WebQSP多了子图
...
def load_data(input_dir, bert_name, batch_size, add_rev=False):
    ...
    if os.path.exists(cache_fn):
        ...
        with open(cache_fn, 'rb') as fp:
            ent2id, rel2id, train_data, test_data = pickle.load(fp)
        print('Train number: {}, test number: {}'.format(
            len(train_data.dataset), len(test_data.dataset)))
    else:
        print('Read data...')
        ent2id, id2name = {}, {}
        for line in open(os.path.join(input_dir, 'entities_id_and_name_merge.txt')):
            split = line.split('\t')
        ...
        train_data = DataLoader(os.path.join(input_dir, 'train_simple.json'), bert_name, ent2id, rel2id, batch_size, id2name, add_rev=add_rev, training=True)
        test_data = DataLoader(os.path.join(input_dir, 'test_simple.json'), bert_name, ent2id, rel2id, batch_size, id2name, add_rev=add_rev)

        with open(cache_fn, 'wb') as fp:
            pickle.dump((ent2id, rel2id, train_data, test_data), fp)

    return ent2id, rel2id, train_data, test_data
